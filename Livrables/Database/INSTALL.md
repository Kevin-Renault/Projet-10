...
-- 2) From this directory run `apply_all.ps1`, which applies the files explicitly in the correct order, including `02_types_enums.sql` then `01_acriss_vehicle.sql` before the core domain.
...
-- 2) Depuis ce dossier exécutez `apply_all.ps1`, qui applique explicitement les fichiers dans le bon ordre, notamment `02_types_enums.sql` puis `01_acriss_vehicle.sql` avant le domaine principal.
