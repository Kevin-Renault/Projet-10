...
1. [Audit de l'existant](#1-audit-de-lexistant)
  - [1.1 Contexte et périmètre](#11-contexte-et-périmètre)
    - [1.1.1 Contexte métier](#111-contexte-métier)
    - [1.1.2 Enjeux et objectifs de l'audit](#112-enjeux-et-objectifs-de-laudit)
    - [1.1.3 Périmètre de l'analyse](#113-périmètre-de-lanalyse)
  - [1.2 Détails de l'audit](#12-détails-de-laudit)
    - [1.2.1 Stack](#121-stack)
    - [1.2.2 Performance et fiabilité](#122-performance-et-fiabilité)
    - [1.2.3 Expérience utilisateur](#123-expérience-utilisateur)
    - [1.2.4 Modèle](#124-modèle)
    - [1.2.5 Analyse des points forts, limites et risques](#125-analyse-des-points-forts-limites-et-risques)
  - [1.3 Conclusion de l'audit](#13-conclusion-de-laudit)
...
#### 1.1.2 Enjeux et objectifs de l’audit
...
#### 1.2.2 performance et fiabilité
...
#### 1.2.5 Analyse des points forts, limites et risques
...
### 2.6.3 Annexe : tables de métadonnées et contraintes clés (raccourci)

Le diagramme ci‑dessous présente la vue métier principale. Pour éviter d'alourdir la vue, plusieurs tables de métadonnées (key/value) et contraintes d'intégrité sont implémentées directement dans la base de données. Ces objets sont décrits et maintenus par les scripts SQL présents dans le dossier [Livrables/Database](Database/).

Principaux objets annexes :
- `payment_metadata`, `webhook_event_payload`, `webhook_event_header`, `chat_metadata`, `chat_text_metadata` — tables key/value normalisées (1NF) pour stocker les métadonnées sans alourdir le modèle principal.
- `refresh_token`, `email_verification`, `service_account`, `agent_profile_permission` — tables opérationnelles d'authentification et de gestion des comptes.

Contraintes et indexes remarquables (exemples) :
- `CHECK (start_at < end_at)` sur `booking` pour garantir la validité des périodes.
- `UNIQUE(payment_provider, provider_payment_id)` sur `payment` pour garantir l'idempotence des paiements.
- Indexes sur `booking(start_at,end_at)`, `vehicle(acriss_code)`, `chat(booking_id)` pour les parcours fréquents.

Voir ci‑dessous l'annexe intégrée listant les tables de métadonnées et les contraintes clés.

## Annexe DB — Tables de métadonnées et contraintes clés

Ce document récapitule les tables "annexes" et contraintes clés implémentées dans la base de données. Les scripts complets se trouvent dans `Livrables/Database/`.

### Tables de métadonnées (key/value)
- `payment_metadata(payment_id, meta_key, meta_value)` — méta données liées au paiement (ex. transaction raw payloads, IDs externes non structurés).
- `webhook_event_payload(webhook_event_id, payload_key, payload_value)` — payloads normalisés en key/value pour faciliter le stockage des événements externes.
- `webhook_event_header(webhook_event_id, header_key, header_value)` — mêmes raisons pour les headers.
- `chat_metadata(chat_id, meta_key, meta_value)` — métadonnées liées à une conversation.
- `chat_text_metadata(chat_text_id, meta_key, meta_value)` — métadonnées liées à un message.

### Tables opérationnelles d'authentification / gestion
- `refresh_token(user_id, token_hash, expires_at, ...)` — gestion des refresh tokens opaques et rotation.
- `email_verification(user_id, code_hash, expires_at, ...)` — codes d'activation à usage unique.
- `service_account(id, name, api_key_hash, ...)` et `service_account_scope(service_account_id, scope)` — comptes de service et scopes.
- `agent_profile_permission(agent_profile_id, permission_id)` — permissions normalisées pour les agents.

### Contraintes & indexes majeurs (exemples)
- `booking`: `CHECK (start_at < end_at)` et index `idx_booking_start_end`.
- `payment`: `UNIQUE(payment_provider, provider_payment_id)` pour garantir l'unicité des paiements importés.
- `vehicle`: `CHECK (acriss_code ~ '^[A-Z0-9]{4}$')` et vue `vehicle_acriss_details` pour reconstruction des labels ACRISS.
- Indexes sur colonnes fréquemment filtrées / triées : `booking(booking_status)`, `vehicle(vehicle_status)`, `chat(booking_id)`.

### Remarques
- Ces objets ont été normalisés pour limiter la duplication et pour conserver la flexibilité des payloads externes (webhooks, providers). Leur présence alourdit la vue conceptuelle mais améliore la conformité 1NF et facilite l'extension sans migration lourde.
- Pour la traçabilité exacte, référez‑vous aux fichiers SQL dans `Livrables/Database/` (notamment `03_auth_schema.sql`, `04_core_domain.sql`, `05_chat.sql`).
